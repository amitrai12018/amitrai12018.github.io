...
Mathematics is not only the study of numbers. It is also the study of structures, relations, transformations, and the properties that remain unchanged under them.{: .prompt-info }
...
numbers, vectors, points, graphs, functions

Operations

What can be done to them?

addition, scaling, composition, concatenation

Relations

How are they compared or connected?

order, distance, adjacency, containment

Transformations

How may their descriptions change?

rotation, relabeling, coordinate change

Laws

Which rules must always hold?

associativity, transitivity, distributivity

Invariants

What should remain unchanged?

distance, connectivity, probability, shape

Equivalence

When are two descriptions structurally the same?

isomorphism, isometry, homeomorphism

A useful first approximation is:

\text{objects}+\text{operations}+\text{relations}+\text{laws}.$$
...
A mathematical symbol receives its meaning from the objects on which it acts, the definition assigned to it, and the laws that definition satisfies.{: .prompt-tip }
...
The first modeling question is not "What mathematics can be used?" but "What must this model allow us to ask and answer?"{: .prompt-info }
...
The important question is not whether an operation can be invented formally. It is whether the operation represents something meaningful in the phenomenon.{: .prompt-tip }
...
Prefer the simplest structure that preserves enough information to answer the question.{: .prompt-tip }
...
$$\text{set}\longrightarrow\text{vector space}\longrightarrow\text{normed vector space}\longrightarrow\text{Banach space}.$$

At each stage, the earlier structure remains and a new property is added.

Similarly:

$$\text{graph}\longrightarrow\text{directed graph}\longrightarrow\text{weighted directed graph}\longrightarrow\text{time-dependent probabilistic graph}.$$
...
concatenate road segments to form paths

Relations

a road connects one intersection to another

Measurements

expected travel time on each road

Transformations

relabeling intersections
...
graph isomorphism preserving directions and weights
...
$$\text{graph}\longrightarrow\text{directed weighted graph}\longrightarrow\text{time-dependent probabilistic directed graph}.$$
...
A model is judged not by whether it reproduces every detail of reality, but by whether it preserves the right structure for its intended purpose.{: .prompt-info }
...
the maps that preserve the structure,

the invariants and equivalences that matter,

the problems the structure is intended to express.

The proposed structure is then tested through examples, theorems, counterexamples, and connections to existing mathematics.

New mathematics is not created by assigning arbitrary symbols. It is created by defining a coherent structure that captures a recurring pattern better than the available language does.{: .prompt-info }
...
Purpose: What must be explained, predicted, compared, or decided?

Objects: What are the relevant entities?

Operations: What can meaningfully be done to them?

Relations: How are they compared, connected, or dependent?

Transformations: How may their representation change?

Invariants: What should remain unchanged?

Equivalence: When should two descriptions count as the same?

Structure: What is the simplest framework that captures these features?

Abstraction: What does the model preserve, and what does it discard?

Validation: Do its conclusions agree with theory, data, and the intended purpose?

This can be condensed into one sentence:

Identify what exists, what can be done to it, how its parts are related, which changes should not matter, and what counts as the same; then choose the simplest structure that preserves enough information to answer the question.{: .prompt-tip }
